...
(function (dashboard, $) {
    ...
    Clients.prototype.onItemClick = function (item) {
        if (item) {
            //document.getElementById("logid").click();
            console.log("onItemClick " + item.clientId)
            //alert(item.clientId);
    ...
    Log.prototype._init = function () {
        var _this = this;
        console.log("_init Log.... " + this.client);
        loading('log.html', function () {
            console.log("loading Log.... " + _this.client);
            _this.vmLog = new Vue({
                el: _this.$html[0],
                data: {
                    connState: false,
    ...
    Log.prototype.loadComplete = function () {
        var _this = this;
        if (!_this.extraldata) {
            alert("extraldata is null");
        } else {
            //将onItemClick携带的clientId设置当前topic
            _this.vmLog.subInfo.topic = "log/" + _this.extraldata.clientId;
        }
        document.getElementById("headId").innerText = _this.extraldata.clientId;
        _this.openLog();
    };
    Log.prototype.show = function (keyword) {
        this.$html.show();
        var _this = this;
        console.log("show  Log.... " + this.client + " " + keyword);
        if (this.client && !this.client.isConnected()) {
            this.disconnect();
        }
        _this.extraldata = keyword;
        if(_this.vmLo){
            _this.loadComplete();
        }
    };
    ...
    Log.prototype.clear = function () {
        var _this = this;
        // _this.client = null;
        _this.vmLog.receiveMsgs = null;
    };

    Log.prototype.newClient = function () {
        this.client = new Paho.MQTT.Client(
            this.vmLog.cInfo.host,
            Number(this.vmLog.cInfo.port),
            this.vmLog.cInfo.clientId);
    };

    Log.prototype.subscribe = function () {
        var _this = this;
        if (!_this.client || !_this.client.isConnected()) {
            alert('subscribe The client does not connect to the broker');
            return;
        }
        if (!_this.vmLog.subInfo.topic) {
            alert('subscribe Please fill in the topic.');
            return;
        }
        this.client.subscribe(_this.vmLog.subInfo.topic, {
            qos: Number(_this.vmLog.subInfo.qos),
            onSuccess: function (msg) {
                console.log(JSON.stringify(msg));
                _this.vmLog.subInfo.time = (new Date()).format("yyyy-MM-dd hh:mm:ss");
                _this.vmLog.subscriptions.push(_this.vmLog.subInfo);
                _this.vmLog.subInfo = {qos: _this.vmLog.subInfo.qos};
            },
    ...
    Log.prototype.scrollUI = function () {
    ...
    Log.prototype.getText = function () {
        var _this = this;
        var arrays = _this.vmLog.receiveMsgs;
        var string = arrays.join("\r\n");
        return string;
    }

    Log.prototype.sendMessage = function (topic, qos, txt) {
        var _this = this;
        _this.vmLog.sendInfo.text = txt;
        _this.vmLog.sendInfo.topic = topic;
        _this.vmLog.sendInfo.qos = qos;
        var text = _this.vmLog.sendInfo.text;
        if (!_this.client || !_this.client.isConnected()) {
            alert('sendMessage The client does not connect to the broker');
            return;
        }
        if (!_this.vmLog.sendInfo.topic) {
        ...
        message.destinationName = _this.vmLog.sendInfo.topic;
        message.qos = Number(_this.vmLog.sendInfo.qos);
        message.retained = _this.vmLog.sendInfo.retained;
        _this.client.send(message);
        _this.vmLog.sendInfo.time = (new Date()).format("yyyy-MM-dd hh:mm:ss");
        _this.vmLog.sendMsgs.push(this.vmLog.sendInfo);
        _this.vmLog.sendInfo = {
            topic: _this.vmLog.sendInfo.topic,
            qos: _this.vmLog.sendInfo.qos,
            retained: _this.vmLog.sendInfo.retained
        };
    };

    Log.prototype.connect = function () {
        var _this = this;
        _this.newClient();
        ...
        _this.client.onMessageArrived = function (message) {
            ...
            _this.vmLog.receiveMsgs.push(message.payloadString);
            _this.scrollUI();
        }
        ...
        var options = {
            ...
            onFailure: function (err) {
                ...
                _this.vmLog.connState = false;
            }
        };
        var userName = _this.vmLog.cInfo.userName;
        var password = _this.vmLog.cInfo.password;
        var keepAlive = _this.vmLog.cInfo.keepAlive;
        var cleanSession = _this.vmLog.cInfo.cleanSession;
        var useSSL = _this.vmLog.cInfo.useSSL;
        if (userName) {
        ...
        console.log("connect " + options.userName + " " + options.password);
        _this.client.connect(options);
    };

    Log.prototype.disconnect = function () {
        var _this = this;
        if (_this.client && _this.client.isConnected()) {
    ...
    Log.prototype.unsubscribe = function () {
        var _this = this;
        if (!_this.client || !_this.client.isConnected()) {
        ...
        this.client.unsubscribe(_this.vmLog.subInfo.topic, {
            onSuccess: function (msg) {
                console.log(JSON.stringify(msg));
                _this.vmLog.subInfo = {qos: _this.vmLog.subInfo.qos};
            },
    ...
    Log.prototype.sendMessage = function () {
        var _this = this;
        var text = _this.vmLog.sendInfo.text;
        if (!_this.client || !_this.client.isConnected()) {
            alert('sendMessage The client does not connect to the broker');
    ...
    Log.prototype.sslPort = function () {
        var useSSL = this.vmLog.cInfo.useSSL;
        if (useSSL) {
            this.vmLog.cInfo.port = 8084
        } else {
            this.vmLog.cInfo.port = 8083
        }
    };
    ...
    Log.prototype.saveLog = function () {
        var _this = this;
        saveLogFile(_this.getText())
    };
    Log.prototype.clearLog = function () {
        var _this = this;
        _this.vmLog.receiveMsgs = [];
    };

    Log.prototype.hide = function () {
        console.log("hide  Log.... " + this.client);
        var _this = this;
        _this.disconnect();
        this.$html.hide();
    };
    ...
    Websocket.prototype.unsubscribe = function () {
        ...
        if (!_this.vmWS.subInfo.topic) {
            alert('unsubscribe Please fill in the topic.');
            return;
        }
